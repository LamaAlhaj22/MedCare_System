...
function patient_update($patientId, $fullName, $phone, $gender, $dob) {
    // Require UserModel to access is_valid_date function
    require_once __DIR__ . '/UserModel.php';
    
    // Check validation constraints
    if ($dob !== '' && $dob !== null && !is_valid_date($dob)) {
        return ['status' => 'error', 'message' => 'Invalid Date of Birth format. Must be YYYY-MM-DD.'];
    }
    if ($gender !== '' && $gender !== null && !in_array($gender, ['Male', 'Female', 'Other'])) {
        return ['status' => 'error', 'message' => 'Invalid gender value selected.'];
    }
